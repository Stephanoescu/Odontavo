...
export default function PatientAppointmentsPage() {
  ...
  const { loadByPatient, upcoming, completed } = useSchedulingStore();
  ...
  const pastList = completed();
  const next = nextList.length > 0 ? nextList[0] : null;
      ...
      {next ? (
        <div className="bg-gradient-to-r from-teal-700 to-teal-600 rounded-2xl p-5 text-white">
          ...
          <div className="flex items-center gap-4">
            <div><p className="text-xs opacity-70">Especialista</p><p className="font-medium">{next.dentistName || 'Tu Dentista'}</p></div>
          </div>
        ...
        <div className="card-base p-8 text-center text-muted-foreground text-sm">
          No tienes ninguna cita próxima programada.
        </div>
      )}
